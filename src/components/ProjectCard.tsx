...
export function ProjectCard({ title, description, tags, github, live, image }: ProjectCardProps) {
  return (
    <article className="flex w-full min-w-0 flex-col items-start gap-8 sm:gap-10 xl:flex-row xl:gap-16">
      {/* Text Content */}
      <div className="w-full max-w-xl flex-[0.8]">
        {/* Tag */}
        ...
        {/* Action Buttons */}
        {((github && !github.startsWith('[GITHUB_REPO') && github !== '') || 
          (live && !live.startsWith('[LIVE_DEMO') && live !== '')) && (
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
            ...
            {live && !live.startsWith('[LIVE_DEMO') && live !== '' && (
              ...
              >
                {live.includes('youtu.be') || live.includes('youtube.com') ? 'Watch Demo' : 'Live Demo'}
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  {live.includes('youtu.be') || live.includes('youtube.com') ? (
                    <>
      ...
      <div className="flex-[1.5] w-full xl:max-w-3xl">
        <div className="relative rounded-xl border border-white/10 bg-[rgb(var(--surface-elevated))] p-1 shadow-2xl overflow-hidden aspect-video w-full">
          ...
          {image ? (
            ...
            <div className="mt-10 flex h-[calc(100%-40px)] flex-col gap-4 bg-[rgb(var(--surface-elevated))] p-4 sm:gap-6 sm:p-6">
